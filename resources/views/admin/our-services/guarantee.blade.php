...
<div class="row mt-3">
  ...
  <div class="row d-flex justify-content-center">
    <div class="col-8 mt-2">
      ...
      @php
        $huarantee_image = json_decode($data->description);
        $g_url = [];
        foreach($huarantee_image as $g_img) {
          array_push($g_url, $g_img->path."/".$g_img->name);
        }
      @endphp
      @if (!empty($g_url[0])) 
        <img src="{{ $g_url[0] }}"  class="rounded mx-auto d-block p-1" alt="..." style="width: 100%">
      @endif 

      <form id="guaranteeImageUpdate" class="text-center" action="{{ route('admin.our-services.guarantee.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
  ...
  <div class="row d-flex justify-content-center mt-3">
    <div class="col-8 text-end">
      ...
      <form id="guaranteeContentsUpdate" class="d-inline" action="{{ route('admin.our-services.guarantee.update') }}" method="POST">
        @csrf
        @method('put')
